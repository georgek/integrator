...
void simple_simplify(node_type **root)
{
     ...
     case op2_type:
          ...
          if (r->u.op2.operand1->type == int_type) {
               ...
               if (r->u.op2.operand1->u.intg.value == 0) {
                    ...
                    case '^':
                         if (((r->u.op2.operand2->type == int_type)
                             ...
                             || ((r->u.op2.operand2->type == float_type)
                                 && (r->u.op2.operand2->u.flot.value == 0.0))
                              ) {
                              /* 0^A = 1 where A = 0 */
                              *root = add_int(1);
                              free_tree(r);
               ...
               else if (r->u.op2.operand2->type == float_type) {
                    /* right operand is a float */
                    switch (r->u.op2.operator) {
                    case '+':
                         *root = add_float(r->u.op2.operand1->u.intg.value +
                                         r->u.op2.operand2->u.flot.value);
                         free_tree(r);
                         break;
                    case '-':
                         *root = add_float(r->u.op2.operand1->u.intg.value -
                    ...
                    case '^':
                         *root = add_float(
                              pow(r->u.op2.operand1->u.intg.value,
                                  r->u.op2.operand2->u.flot.value));
                         free_tree(r);
                         break;
                    default:
          ...
          else if (r->u.op2.operand1->type == float_type) {
               /* left operand is a float */
               if (r->u.op2.operand1->u.flot.value == 0.0) {
                    /* left operand is zero */
                    switch (r->u.op2.operator) {
                    case '+':
                         /* 0+x = x */
                    ...
                    switch (r->u.op2.operator) {
                    case '+':
                         *root = add_float(r->u.op2.operand1->u.flot.value +
                                           r->u.op2.operand2->u.intg.value);
                         free_tree(r);
                         break;
                    case '-':
                    ...
                    case '/':
                         *root = add_float(r->u.op2.operand1->u.flot.value /
                         ...
                         break;
                    case '^':
                         *root = add_float(pow(r->u.op2.operand1->u.flot.value,
                                               r->u.op2.operand2->u.intg.value));
                         free_tree(r);
                         break;
                    default:
                    ...
                    case '/':
                         *root = add_float(
                              r->u.op2.operand1->u.flot.value /
                              r->u.op2.operand2->u.flot.value);
                         free_tree(r);
                         break;
                    case '^':
                         *root = add_float(
                              pow(r->u.op2.operand1->u.flot.value,
                                  r->u.op2.operand2->u.flot.value));
                         free_tree(r);
                         break;
                    default:
               ...
               else if (r->u.op2.operand1->u.flot.value == 1.0) {
                    /* left operand is 1 */
                    switch (r->u.op2.operator) {
                    case '*':
                         /* 1*x = x */
               ...
               if (r->u.op2.operand2->type == float_type) {
                    /* right operand is a float */
                    if (r->u.op2.operand2->u.flot.value == 0) {
                         /* right operand is 0 */
                         switch (r->u.op2.operator) {
                         case '+':
                              /* x+0 = x */
                    ...
                    else if (r->u.op2.operand2->u.flot.value == 1) {
                         /* right operand is 1 */
                         switch (r->u.op2.operator) {
                         case '*':
                              /* x*1 = x */
     ...
     case op1_type:
          ...
          if (r->u.op1.operator == UMINUS) {
               ...
               else if (r->u.op1.operand->type == float_type) {
                    *root = add_float(-(r->u.op1.operand->u.flot.value));
                    free_tree(r);
               }
          }
          break;
